...
namespace BVSoftware.Commerce.Catalog
{
    public class CategoryRepository : ConvertingRepositoryBase<Data.EF.bvc_Category, Category>
    {
        ...
        public override bool  Create(Category item)
        {
            ...
            bool rewriteUrlInUse = BVSoftware.Commerce.Utilities.UrlRewriter.IsCategorySlugInUse(item.RewriteUrl, item.Bvin, this);
            for (int i = 0; i < 10; i++)
            {
                if (rewriteUrlInUse)
                {
                    item.RewriteUrl = item.RewriteUrl + "-2";
                    rewriteUrlInUse = BVSoftware.Commerce.Utilities.UrlRewriter.IsCategorySlugInUse(item.RewriteUrl, item.Bvin, this);
