...
namespace BVSoftware.Commerce.Utilities
{
    public class TagReplacer
    {
        public static string ReplaceContentTags(string source, BVApplication bvapp, string itemCount, bool isSecureRequest)            
        {
            ...
            output = output.Replace("{{assets}}", BVSoftware.Commerce.Storage.DiskStorage.BaseStoreThemeUrl(currentStore.Id, currentStore.Settings.ThemeId, isSecureRequest) + "assets/");
            output = output.Replace("{{img}}", BVSoftware.Commerce.Storage.DiskStorage.BaseStoreUrl(currentStore.Id, isSecureRequest) + "storeassets/");
