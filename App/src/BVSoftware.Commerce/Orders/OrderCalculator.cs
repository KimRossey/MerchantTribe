...
namespace BVSoftware.Commerce.Orders
{
    public class OrderCalculator: IOrderCalculator
    {
        ...
        private void ApplyVolumeDiscounts(Order o)
        {
            ...
            {
                List<BVSoftware.Commerce.Catalog.ProductVolumeDiscount> volumeDiscounts = _app.CatalogServices.VolumeDiscounts.FindByProductId(id);                    
                int quantity = quantities[id];
                BVSoftware.Commerce.Catalog.ProductVolumeDiscount volumeDiscountToApply = null;
                if (volumeDiscounts.Count > 0)
                {
                    // Locate the correct discount in the chart of discounts
                    foreach (BVSoftware.Commerce.Catalog.ProductVolumeDiscount volumeDiscount in volumeDiscounts)
                    {
